...
def _get_bootstrap_frame(depth) -> Tuple[Optional[FrameType], bool]:
    try:
...
def _get_unhandled_exception_frame(depth) -> Optional[FrameType]:
    try:
...
class ThreadInfo:
    ...
    def __init__(self, thread, trace, additional_info):
        self.thread = thread
...
def _return_event(code, instruction, retval):
    ...
    if step_cmd in (CMD_STEP_INTO, CMD_STEP_INTO_MY_CODE, CMD_STEP_INTO_COROUTINE):
        ...
        if frame.f_back is not None and not info.pydev_use_scoped_step_frame:
            ...
            if (
                # Not filtered out.
                not back_func_code_info.always_skip_code and not back_func_code_info.always_filtered_out
    ...
    if step_cmd in (CMD_STEP_RETURN, CMD_STEP_RETURN_MY_CODE) and _is_same_frame(info, stop_frame, frame):
        ...
        if f_back is not None:
            ...
            if back_func_code_info is not None and not back_func_code_info.always_skip_code \
