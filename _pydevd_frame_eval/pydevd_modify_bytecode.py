...
_PREDICT_TABLE = {
    ...
    # the line number actually changes from one intsruction to the predicted one).
}

# 3.9/3.10 optimizes things so that some branches may be copied multiple times (for instance
# if the body of a finally has a single assign statement it can copy that to the case
# where an exception happens and doesn't happen for optimization purposes) and as such
# we need to add the programattic breakpoint multiple times.
