...
def get_instructions_to_add(stop_at_line):
    '''
...
class _HelperBytecodeList(object):
    ...
    to keep track of indices that change).
    '''
...
def insert_pydevd_breaks(code_to_modify, breakpoint_lines, code_line_info=None):
    """
    ...
    # now, since we generate a spurious line event, but it's a bit pointless
    # as we'll stop in the first line and we don't stop tracing after the
    # user resumes (in the future, if we do that we can remove this,
    # but to do this we need to handle breakpoints on jumps better
    # in the bytecode generation).
            ...
            if instruction_lineno is not None:
                ...
                if instruction_lineno in modified_breakpoint_lines:
                    ...
                    if curr_node.prev is not None and curr_node.prev.data.__class__ == Label \
                        ...
                        for new_instruction in get_instructions_to_add(instruction_lineno):
...
# Enable old version to compare the generated bytecode.
def _OLD_insert_pydevd_breaks(code_to_modify, breakpoint_lines, code_line_info=None):
    from _pydevd_frame_eval.pydevd_modify_bytecode_old import old_insert_pydevd_breaks
    success, code_obj = old_insert_pydevd_breaks(code_to_modify, breakpoint_lines, code_line_info)
    if success:
        op_number = debug_helper.write_dis(code_to_modify, prefix='old.')
        debug_helper.write_dis(code_obj, op_number, prefix='old.')
    return success, code_obj
