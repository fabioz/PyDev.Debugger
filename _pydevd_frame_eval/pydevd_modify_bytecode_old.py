...
def old_insert_pydevd_breaks(code_to_modify, breakpoint_lines, code_line_info=None):
    c = code_to_modify
    for line in reversed(sorted(breakpoint_lines)):
        code_to_insert = create_pydev_trace_code_wrapper(line)
        success, c = insert_code(code_to_modify, code_to_insert, line, code_line_info)
        if not success:
