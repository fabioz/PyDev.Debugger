...
def test_suspended_frames_manager_global_context():
    from _pydevd_bundle.pydevd_suspended_frames import SuspendedFramesManager
    suspended_frames_manager = SuspendedFramesManager()
    py_db = _DummyPyDB()
    tracker = suspended_frames_manager.get_global_frames_tracker(py_db)
    # : :type tracker: _GlobalFramesTracker
    global_frame = get_frame()

    value = global_frame.f_locals['var2']
    tracker.obtain_as_variable('var2', value, frame=global_frame)
    variable = suspended_frames_manager.get_variable(id(value))
    print([(x.get_name(), x.get_var_data()) for x in variable.get_children_variables()])
    #
    # # Should be properly sorted.
    # assert ['var1', 'var2', 'var3'] == [x.get_name()for x in variable.get_children_variables()]
    #
