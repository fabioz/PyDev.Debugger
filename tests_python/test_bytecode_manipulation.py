...
pytestmark = pytest.mark.skipif(True, reason='UNFINISHED! TODO!')
...
def check(filename, method, method_kwargs=None, skip_breaks_at_lines=None, method_to_change=None):
    '''
    :param filename:
    :param method:
    :param method_kwargs:
    ...
    try:
        method_to_change.__globals__['_pydev_stop_at_break'] = _pydev_stop_at_break
        ...
        for line in sorted(code_line_info.line_to_offset):
            ...
            # callback got called).
            success, new_code = pydevd_modify_bytecode.insert_pydevd_breaks(code, set([line]))
            assert success
...
def test_set_pydevd_break_06(pyfile):
    ...
    check('_bytecode_super.py', _bytecode_super.B, method_to_change=_bytecode_super.B.__init__)
