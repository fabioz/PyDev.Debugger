...
class PyDevdAPI(object):
    ...
    def absolute_filename(self, filename):
        return pydevd_file_utils.absolute_path(filename)
    ...
    def add_breakpoint(
        ...
        if multi_mapping_applied:
            ...
            translated_filename = source_mapped_filename
            canonical_normalized_filename = source_mapped_filename
            line = new_line

        else:
            canonical_normalized_filename = self.canonical_normalized_filename(translated_filename)

            if adjust_line and not translated_filename.startswith('<'):
                # Validate breakpoints and adjust their positions.
                try:
                    lines = sorted(_get_code_lines(translated_filename))
                except Exception:
                    pass
                else:
        ...
        if not translated_filename.startswith('<'):
            # Note: if a mapping pointed to a file starting with '<', don't validate.

            if not pydevd_file_utils.exists(translated_filename):
            ...
            if (
                    ...
                    py_db.apply_files_filter(self._DummyFrame(translated_filename), translated_filename, False)
                ):
    ...
    def set_source_mapping(self, py_db, source_filename, mapping):
        ...
        absolute_source_filename = self.absolute_filename(source_filename)
