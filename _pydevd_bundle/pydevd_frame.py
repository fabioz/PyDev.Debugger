        ...
        try:
            ...
            if main_debugger.ignore_exceptions_thrown_in_lines_with_ignore_exception:
                for check_trace_obj in (initial_trace_obj, trace_obj):
                    absolute_filename = get_abs_path_real_path_and_base_from_frame(check_trace_obj.tb_frame)[0]

                    filename_to_lines_where_exceptions_are_ignored = self.filename_to_lines_where_exceptions_are_ignored

                    lines_ignored = filename_to_lines_where_exceptions_are_ignored.get(absolute_filename)
                    if lines_ignored is None:
                        lines_ignored = filename_to_lines_where_exceptions_are_ignored[absolute_filename] = {}
                    ...
                    from_user_input = main_debugger.filename_to_lines_where_exceptions_are_ignored.get(absolute_filename)
                    if from_user_input:
                        merged = {}
